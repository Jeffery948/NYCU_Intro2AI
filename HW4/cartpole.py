...
class Agent():
    ...
    def init_bins(self, lower_bound, upper_bound, num_bins):
        ...
        return bins[1:] # Use linspace to spilt into 7 parts and do not take first part
    ...
    def discretize_value(self, value, bins):
        ...
        return np.digitize(value, bins) # Use digitize to discretize value
    ...
    def discretize_observation(self, observation):
        ...
        dis_value = () # Define a empty tuple
        for obs, bin in zip(observation, self.bins):
            dis_value += (self.discretize_value(obs, bin),) # Discretized the 4 features of observation
        return dis_value
    ...
    def choose_action(self, state):
        ...
        action = np.argmax(self.qtable[state]) # Choose best action by Q-table
        if np.random.rand() >= self.epsilon: # Decide whether to explore
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        state_action_pair = state + (action,) # Use tuple to be Q-table's index
        self.qtable[state_action_pair] = (1 - self.learning_rate) * self.qtable[state_action_pair] + \
        self.learning_rate * (reward + self.gamma * np.max(self.qtable[next_state])) # Update Q-table
    ...
    def check_max_Q(self):
        ...
        initial_state = self.discretize_observation(self.env.reset()) # Discretize the initial state
        return np.max(self.qtable[initial_state]) # Calculate max Q-value of initial state
