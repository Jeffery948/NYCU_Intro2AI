...
class Agent():
    ...
    def learn(self):
        ...
        # 2. Sample trajectories of batch size from the replay buffer and make some modifications.
        state_batch, action_batch, reward_batch, next_state_batch, mask_batch = self.buffer.sample(self.batch_size)
        state_batch = torch.Tensor(state_batch)
        action_batch = torch.LongTensor(action_batch).reshape(self.batch_size, 1)
        reward_batch = torch.Tensor(reward_batch).reshape(self.batch_size, 1)
        next_state_batch = torch.Tensor(next_state_batch)
        mask_batch = torch.Tensor(mask_batch).reshape(self.batch_size, 1)

        # 3. Forward the data to the evaluate net and the target net and make some modifications.
        q_eval = self.evaluate_net(state_batch).gather(1, action_batch)
        next_q = self.target_net(next_state_batch)
        next_q = torch.Tensor([max(q) for q in next_q]).reshape(self.batch_size, 1)
        q_target = reward_batch + self.gamma * next_q * (1 - mask_batch)
        ...
        # 7. Optimize the loss function and update count.
        self.optimizer.step()
        self.count += 1
    ...
    def check_max_Q(self):
        ...
        initial_state = torch.Tensor(self.env.reset())
        q_value = self.target_net(initial_state)
        return float(max(q_value)) # Calculate max Q-value of initial state
